...
void FrameEvaluator::EvaluateFrameReconstruction(Frame& frame,
  ...
  auto transformed_landmark_positions = TransformPointCloud(
  ...
  SaveGroundTruthToFrame(frame, scale_factor, landmarks_ground_truth,
                         indices_in_frame);

  computed_rmse_.push_back(computed_error);
}

std::tuple<float, float> FrameEvaluator::ComputeReconstructionRMSE(
    const std::vector<Eigen::Vector3f>& reconstruction_landmarks,
  ...
  if (align_scales) {
    ...
    return ComputeRMSEWithoutScaleAlignment(estimated_depths,
                                            ground_truth_depths);
  }
}

std::tuple<float, float> FrameEvaluator::ComputeRMSEWithoutScaleAlignment(
    const std::vector<float>& estimated_depths,
  ...
  float interquartileRange =
      sorted_errors[(int)(sorted_errors.size() * 0.75f)] -
      sorted_errors[(int)(sorted_errors.size() * 0.25f)];
  ...
  Eigen::VectorXf residuals =
      ground_truth_depths_eigen - estimated_depths_eigen;
  Eigen::VectorXf squared_residuals =
      (residuals.array() * residuals.array()).matrix();
  ...
  return make_tuple(
      sqrt(inlier_residuals.dot(inlier_residuals) / (float)n_inliers), 1.f);
}

std::tuple<float, float> FrameEvaluator::ComputeRMSEWithScaleAlignment(
    const std::vector<float>& estimated_depths,
    const std::vector<float>& ground_truth_depths) {
  int n_depths = estimated_depths.size();
  ...
  for (int it = 0; it < 10; it++) {
    // Get all errors.
    Eigen::VectorXf squared_residuals =
        ((ground_truth_depths_eigen - scale_factor * estimated_depths_eigen)
             .array() *
         (ground_truth_depths_eigen - scale_factor * estimated_depths_eigen)
             .array())
            .matrix();
    ...
    sort(squared_residuals_sorted.data(),
         squared_residuals_sorted.data() + n_depths);
    ...
    for (int idx = 0; idx < n_depths; idx++) {
      if (squared_residuals(idx) <= th) {
        ...
        inlier_residuals(current_idx) = residuals(idx);
        inlier_jacobians(current_idx) =
            -residuals(idx) * estimated_depths_eigen(idx);
    ...
    Eigen::VectorXf aligned_residuals =
        inlier_groundtruth_depths - scale_factor * inlier_depths;
...
std::vector<Eigen::Vector3f> FrameEvaluator::TransformPointCloud(
...
std::vector<absl::StatusOr<Eigen::Vector3f>> FrameEvaluator::ComputeGroundTruth(
  ...
  if (!options_.precomputed_depth_) {
    auto raw_ground_truth =
        stereo_matcher_->ComputeStereo3D(keypoints, im_left, im_right);
    ...
    for (auto keypoint : keypoints) {
      float ground_truth_depth = Interpolate(
          keypoint.pt.x, keypoint.pt.y, im_right.ptr<float>(0), im_right.cols);
...
void FrameEvaluator::SaveGroundTruthToFrame(
    Frame& frame, const float scale_factor,
    const std::vector<absl::StatusOr<Eigen::Vector3f>>& ground_truth,
    const std::vector<int> indices_in_frame) {
  std::vector<absl::StatusOr<Eigen::Vector3f>>& frame_ground_truth =
      frame.MutableGroundTruth();
  fill(frame_ground_truth.begin(), frame_ground_truth.end(),
       absl::InternalError("No ground truth available."));

  Sophus::SE3f world_transform_camera =
      frame.CameraTransformationWorld().inverse();

  for (int idx = 0; idx < ground_truth.size(); idx++) {
    if (ground_truth[idx].ok()) {
      const int frame_index = indices_in_frame[idx];
      frame_ground_truth[frame_index] =
          world_transform_camera * (*ground_truth[idx] / scale_factor);
    }
  }
}
