...
float RaysParallaxCosine(const Eigen::Vector3f& ray_1,
                         const Eigen::Vector3f& ray_2);
...
// Triangulates a 3D point form the camera poses and the normalized bearing
// rays. Lee, Seong Hun, and Javier Civera. "Triangulation: Why Optimize?."
// arXiv preprint arXiv:1907.11917 (2019).
absl::StatusOr<Eigen::Vector3f> TriangulateMidPoint(
    const Eigen::Vector3f& ray_1, const Eigen::Vector3f& ray_2,
    const Sophus::SE3f& camera1_transform_world,
    const Sophus::SE3f& camera2_transform_world);
...
float Interpolate(const float x, const float y, const T* mat, const int cols) {
  ...
  return (float)(mat[(int)y_ * cols + (int)x_]) * w00 +
         (float)(mat[(int)y_ * cols + (int)x_ + 1]) * w10 +
         (float)(mat[((int)y_ + 1) * cols + (int)x_]) * w01 +
         (float)(mat[((int)y_ + 1) * cols + (int)x_ + 1]) * w11;
}
