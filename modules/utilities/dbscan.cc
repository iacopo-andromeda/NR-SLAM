...
std::vector<int> Dbscan2D(std::vector<cv::Point2f>& points) {
  mlpack::DBSCAN<RangeSearch<>, OrderedPointSelection> clustering(0.2, 3);
...
std::vector<int> Dbscan3D(std::vector<Eigen::Vector3f>& points) {
  mlpack::DBSCAN<RangeSearch<>, OrderedPointSelection> clustering(
      2.5, 5);  // (4.5, 5) for Hamlyn 20, (2.5, 5) for Hamlyn 21
  ...
  vector<pair<int, int>> vOrderedClusters(mClusterSize.begin(),
                                          mClusterSize.end());
...
std::vector<int> DbscanND(std::vector<Eigen::VectorXf>& points) {
  ...
  mlpack::DBSCAN<RangeSearch<>, OrderedPointSelection> clustering(epsilon, 10);
