...
class FrameEvaluator {
 public:
  ...
  void EvaluateFrameReconstruction(Frame& frame, const cv::Mat& im_left,
                                   const cv::Mat& im_right);

  void SaveResultsToFile();

 private:
  std::vector<absl::StatusOr<Eigen::Vector3f>> ComputeGroundTruth(
      const std::vector<cv::KeyPoint>& keypoints, const cv::Mat& im_left,
      const cv::Mat& im_right, std::shared_ptr<CameraModel> calibration);

  std::tuple<float, float> ComputeReconstructionRMSE(
      const std::vector<Eigen::Vector3f>& reconstruction_landmarks,
      const std::vector<absl::StatusOr<Eigen::Vector3f>>&
  ...
  std::tuple<float, float> ComputeRMSEWithoutScaleAlignment(
      const std::vector<float>& estimated_depths,
      const std::vector<float>& ground_truth_depths);

  std::tuple<float, float> ComputeRMSEWithScaleAlignment(
      const std::vector<float>& estimated_depths,
      const std::vector<float>& ground_truth_depths);

  void SaveGroundTruthToFrame(
