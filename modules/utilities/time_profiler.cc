...
void TimeProfiler::Toc(std::string identifier) {
  ...
  float timeElapsed = std::chrono::duration_cast<std::chrono::milliseconds>(
                          end - initial_measurements_[identifier])
                          .count();
  watchers_[identifier].push_back(timeElapsed);
}

void TimeProfiler::PrintStatisticsForIdentifier(
  ...
  LOG(INFO).NoPrefix() << "Time statistics for identifier \"" << identifier
                       << "\":";
  LOG(INFO).NoPrefix() << "\t-Number of samples: "
                       << watchers_.at(identifier).size();
...
void TimeProfiler::SaveStatisticsToFile(const std::string& file_name) {
  ...
  for (const auto& [identifier, data] : watchers_) {
    time_data_writer << identifier << ": " << Mean(watchers_.at(identifier))
                     << " " << Sigma(watchers_.at(identifier)) << " ";
