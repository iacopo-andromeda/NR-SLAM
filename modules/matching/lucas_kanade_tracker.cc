...
LucasKanadeTracker::LucasKanadeTracker(const cv::Size winSize,
                                       const int maxLevel, const int maxIters,
                                       const float epsilon,
                                       const float minEigThreshold)
    : winSize_(winSize),
      maxLevel_(maxLevel),
      maxIters_(maxIters),
      epsilon_(epsilon),
...
void LucasKanadeTracker::SetReferenceImage(const Mat& refIm,
  ...
  // Compute reference windows (intensity and derivtives) and means of the
  // windows
  ...
  for (int level = maxLevel_; level >= 0; level--) {
    ...
    int cn = I.channels(),
        cn2 = cn * 2;  // cn should be 1 therefor cn2 should be 2
    ...
    Mat derivIWinBuf(winSize_, CV_MAKETYPE(derivDepth, cn2),
                     (*_buf) + winSize_.area() * cn);
    ...
    for (int i = 0; i < prevPts_.size(); i++) {
      ...
      for (y = 0; y < winSize_.height; y++) {
        ...
        const short* dsrc =
            derivI.ptr<short>() + (y + ipoint.y) * dstep + ipoint.x * 2;
...
int LucasKanadeTracker::Track(const Mat& newIm, std::vector<KeyPoint>& nextPts,
                              vector<LandmarkStatus>& vMatched,
  ...
  for (int level = maxLevel_; level >= 0; level--) {
    ...
    // Integration window buffers
    Mat IWinBuf(winSize_, CV_MAKETYPE(derivDepth, cn), (*_buf));
    Mat JWinBuf(winSize_, CV_MAKETYPE(derivDepth, cn),
                (*_buf) + winSize_.area());
    Mat derivIWinBuf(winSize_, CV_MAKETYPE(derivDepth, cn2),
                     (*_buf) + 2 * winSize_.area());
    Mat derivJWinBuf(winSize_, CV_MAKETYPE(derivDepth, cn2),
                     (*_buf) + 4 * winSize_.area());
    ...
    for (int i = 0; i < prevPts_.size(); i++) {
      ...
      for (j = 0; j < maxIters_; j++) {
        ...
        for (y = 0; y < winSize_.height; y++) {
          ...
          const short* dsrc =
              derivJ.ptr<short>() + (y + inextPt.y) * dstep + inextPt.x * cn2;
          ...
          for (; x < winSize_.width * cn; x++, dsrc += 2, dJptr += 2) {
            ...
            int jval = CV_DESCALE(src[x] * jw00 + src[x + cn] * jw01 +
                                  ...
                                  W_BITS1 - 5);
            int jxval =
                CV_DESCALE(dsrc[0] * jw00 + dsrc[cn2] * jw01 +
                               dsrc[dstep] * jw10 + dsrc[dstep + cn2] * jw11,
                           W_BITS1);
            int jyval = CV_DESCALE(dsrc[1] * jw00 + dsrc[cn2 + 1] * jw01 +
                                       dsrc[dstep + 1] * jw10 +
        ...
        float minEig =
            (A22 + A11 -
             std::sqrt((A11 - A22) * (A11 - A22) + 4.f * A12 * A12)) /
            (2 * winSize_.width * winSize_.height);

        if (minEig < minEigThreshold_ || D < FLT_EPSILON) {
          if (level == 0) vMatched[i] = BAD_FEATURE;
          continue;
        }
        ...
        if (delta.ddot(delta) <= epsilon_) break;
  ...
  const float N_inv = 1.f / (float)winSize_.area(),
              N_inv_1 = 1.f / (float)(winSize_.area() - 1);
    ...
    if (IsUsable(vMatched[i])) {
      ...
      for (y = 0; y < winSize_.height; y++) {
        ...
        for (; x < winSize_.width * cn; x++) {
          ...
          // Get sub pixel values from images
          int jval =
              CV_DESCALE(src[x] * jw00 + src[x + cn] * jw01 +
...
LucasKanadeTracker::PhotometricInformation
LucasKanadeTracker::GetPhotometricInformationOfPoint(const int idx) {
  ...
  for (int level = 0; level < vMeanI_.size(); level++) {
    ...
    photometric_information.squared_mean_gray_per_level.push_back(
        vMeanI2_[level][idx]);
...
void LucasKanadeTracker::InsertPhotometricInformation(
    cv::KeyPoint& keypoint,
    LucasKanadeTracker::PhotometricInformation& photometric_information) {
  prevPts_.push_back(keypoint);
  for (int level = 0; level <= maxLevel_; level++) {
    vMeanI_[level].push_back(
        photometric_information.mean_gray_per_level[level]);
    vMeanI2_[level].push_back(
        photometric_information.squared_mean_gray_per_level[level]);
    Iref_[level].push_back(
        photometric_information.gray_reference[level].clone());
    Idref_[level].push_back(
        photometric_information.gradient_reference[level].clone());
  }
}
