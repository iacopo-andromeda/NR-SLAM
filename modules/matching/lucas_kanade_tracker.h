...
class LucasKanadeTracker {
 public:
   ...
   */
  LucasKanadeTracker(const cv::Size _winSize, const int _maxLevel,
                     const int _maxIters, const float _epsilon,
                     const float _minEigThreshold);
   ...
   */
  void SetReferenceImage(const cv::Mat& refIm,
                         const std::vector<cv::KeyPoint>& refPts,
  ...
  void InsertPhotometricInformation(
      cv::KeyPoint& keypoint, PhotometricInformation& photometric_information);
 ...
 public:
  ...
  //-------------------------------
  cv::Size
      winSize_;  // size of the integration window of the Lucas-Kanade algorithm
  int maxLevel_;   // max level of the image pyramids
  int maxIters_;   // max number of iterations of the optical flow algorithm
  float epsilon_;  // minimum optical flow imposed displacement. If lower, we
                   // stop computing
  float minEigThreshold_;  // min eigen threshold value for the Spatial Gradient
                           // matrix
  ...
  std::vector<cv::KeyPoint>
      prevPts_;  // Original coordinates of the points to track
  std::vector<std::vector<float>> vMeanI_;  // Reference window mean intensities
  std::vector<std::vector<float>>
      vMeanI2_;  // Reference window mean squared intensities
