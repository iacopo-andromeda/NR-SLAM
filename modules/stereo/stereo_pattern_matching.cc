...
StereoPatternMatching::StereoPatternMatching(
    std::shared_ptr<CameraModel> calibration, float baseline)
    : StereoMatcher(calibration, baseline) {}

absl::StatusOr<Eigen::Vector3f> StereoPatternMatching::computeStereo3D(
    const cv::KeyPoint& kp, const cv::Mat& imLeft, const cv::Mat& imRight) {
  if (kp.pt.x < 0 || kp.pt.y < 0 || kp.pt.y > imRight.rows - 20 ||
      kp.pt.x > imRight.cols - 20) {
  ...
  cv::Point minLoccrop;
  cv::Point maxLoccrop;
  cv::minMaxLoc(tmp, &minValcrop, &maxValcrop, &minLoccrop, &maxLoccrop,
                cv::Mat());
  if (maxValcrop > 250)
    return absl::InternalError("Feature out of image boundaries");
...
StereoPatternMatching::ComputeStereo3D(const std::vector<cv::KeyPoint>& kp,
                                       const cv::Mat& imLeft,
