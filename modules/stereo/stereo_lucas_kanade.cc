...
std::vector<absl::StatusOr<Eigen::Vector3f>> StereoLucasKanade::ComputeStereo3D(
  ...
  for (int idx = 0; idx < keypoints.size(); idx++) {
    if (landmark_statuses[idx] == TRACKED) {
      ...
      if (dif_in_rows > 2.0) {
        computed3D.push_back(absl::InternalError("Rows are different: " +
                                                 to_string(dif_in_rows)));
      } else {
        float disparity =
            (abs(keypoints[idx].pt.x - keypoints_right[idx].pt.x));

        CHECK(!isnan(disparity) && !isinf(disparity));

        Eigen::Vector3f landmark_position;
        landmark_position.x() =
            baseline_ / disparity * (((float)keypoints[idx].pt.x - cx) / fx);
        landmark_position.y() =
            baseline_ / disparity * (((float)keypoints[idx].pt.y - cy) / fy);
