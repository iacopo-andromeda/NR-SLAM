...
RegularizationGraph::RegularizationGraph(Options& options, Map* map)
  ...
  min_weight_ =
      InterpolationWeight(options_.weight_sigma * 1.5, options_.weight_sigma);
}
...
bool EdgeComparator(
    pair<ID, std::shared_ptr<RegularizationGraph::Edge>> edge_1,
...
bool RegularizationGraph::UpdateConnection(
  ...
  if (graph_[mappoint_id_1].contains(mappoint_id_2) &&
    ...
    shared_ptr<RegularizationGraph::Edge> edge =
        graph_[mappoint_id_1][mappoint_id_2];
    ...
    edge->weight =
        InterpolationWeight(edge->max_distance, options_.weight_sigma);
    edge->last_relative_position = relative_position;

    // Edge prune criteria.
    if (fabs((edge->max_distance - edge->min_distance) / edge->min_distance) >
        options_.streching_th) {
...
int RegularizationGraph::UpdateVertex(ID mappoint_id) {
  ...
  for (auto& [other_mappoint_id, edge] : connections) {
    ...
    Eigen::Vector3f other_landmark_position =
        map_->GetMapPoint(other_mappoint_id)->GetLastWorldPosition();

    if (UpdateConnection(mappoint_id, other_mappoint_id, landmark_position,
                         other_landmark_position))
      n_good_connections++;
  }
...
std::string RegularizationGraph::ToString() {
  ...
  for (const auto& [id, connections] : graph_) {
    for (const auto& [id_other, edge] : connections) {
      graph_string += to_string(id) + " " + to_string(id_other) + " " +
...
void RegularizationGraph::GetOptimizationNeighbours(
    const std::vector<ID>& mappoints_ids, const int connections_per_point,
  ...
  for (auto mappoint_id : mappoints_ids) {
    auto connections = this->GetEdges(mappoint_id);
    ...
    for (const auto& [mappoint_id_other, regularization_edge] : connections) {
      ...
      if (!zero_order_connections.contains(mappoint_id_other)) {
        if (!first_order_connections.contains(mappoint_id_other)) {
          first_order_connections[mappoint_id_other] =
              absl::flat_hash_set<ID>();
        }
