...
MapPoint::MapPoint(const Eigen::Vector3f& landmark_position,
...
void MapPoint::SetPhotometricInformation(
...
MapPoint::GetPhotometricInformation() {
  return photometric_information_;
}

int MapPoint::GetKeyPointId() { return keypoint_id_; }

bool& MapPoint::IsActive() { return is_active_; }
...
std::vector<Eigen::Vector3f> MapPoint::GetLandmarkFlow(const int n) {
  ...
  if (landmark_position_history_.size() < n) {
    ...
    start_iterator = landmark_position_history_.begin() +
                     (landmark_position_history_.size() - n);
  }

  auto end_iterator =
      landmark_position_history_.begin() + (landmark_position_history_.size());
  return vector<Eigen::Vector3f>(start_iterator, end_iterator);
}

long unsigned int MapPoint::GetId() { return id_; }
