...
EssentialMatrixInitialization::EssentialMatrixInitialization(
    Options& options, std::shared_ptr<CameraModel> calibration,
...
void EssentialMatrixInitialization::ChangeReference(
    std::vector<cv::KeyPoint>& keypoints) {
  reference_keypoints_ = keypoints;
}

absl::Status EssentialMatrixInitialization::Initialize(
  ...
  fill(landmarks_position.begin(), landmarks_position.end(),
       absl::InternalError("Not triangulated"));

  // If enough features are tracked, try to find an Essential matrix with
  // RANSAC.
  vector<bool> inliers_of_E(n_matches, false);
  int n_inliers_of_E;
  Eigen::Matrix3f E =
      FindEssentialWithRANSAC(n_matches, inliers_of_E, n_inliers_of_E);

  // Reconstruct the environment with the Essential matrix found.
  auto status =
      ReconstructEnvironment(E, camera_transform_world, landmarks_position,
                             n_inliers_of_E, inliers_of_E);
  ...
  for (int idx = 0; idx < feature_tracks_statuses_.size(); idx++) {
    if (feature_tracks_statuses_[idx] == TRACKED) {
      reference_rays_.block(n_tracked_points, 0, 1, 3) =
          calibration_
              ->Unproject(reference_keypoints_[idx].pt.x,
                          reference_keypoints_[idx].pt.y)
              .normalized();
      current_rays_.block(n_tracked_points, 0, 1, 3) =
          calibration_
              ->Unproject(current_keypoints_[idx].pt.x,
                          current_keypoints_[idx].pt.y)
              .normalized();
...
Eigen::Matrix3f EssentialMatrixInitialization::FindEssentialWithRANSAC(
    const int n_matches, vector<bool>& inliers, int& n_inliers) {
  int best_score = 0;
  ...
  cv::kmeans(reference_keypoints_tracked_, n_clusters, labels,
             termination_criteria, n_attemps, cv::KMEANS_PP_CENTERS, centers);
  ...
  while (current_iteration < max_iterations) {
    ...
    for (int idx = 0; idx < n_clusters; idx++) {
      ...
      reference_rays_sample.block<1, 3>(idx, 0) =
          reference_rays_.block(feature_idx, 0, 1, 3);
      current_rays_sample.block<1, 3>(idx, 0) =
          current_rays_.block(feature_idx, 0, 1, 3);
    }

    // Compute model with the sample.
    Eigen::Matrix<float, 3, 3> E =
        ComputeE(reference_rays_sample, current_rays_sample);
  ...
  for (int idx = 0; idx < inliers_best_model.size(); idx++) {
    if (inliers_best_model[idx]) {
      best_model_reference_rays.block<1, 3>(current_idx, 0) =
          reference_rays_.block(idx, 0, 1, 3);
      best_model_current_rays.block<1, 3>(current_idx, 0) =
          current_rays_.block(idx, 0, 1, 3);
      current_idx++;
    }
  }
...
Eigen::Matrix3f EssentialMatrixInitialization::ComputeE(
    Eigen::Matrix<float, 8, 3>& reference_rays_sample,
    Eigen::Matrix<float, 8, 3>& current_rays_sample) {
  // Compute a first estimation of the Essential matrix.
  Eigen::Matrix<float, 8, 9> A;
  for (int idx = 0; idx < 8; idx++) {
    A.block<1, 3>(idx, 0) =
        reference_rays_sample.row(idx) * current_rays_sample(idx, 0);
    A.block<1, 3>(idx, 3) =
        reference_rays_sample.row(idx) * current_rays_sample(idx, 1);
    A.block<1, 3>(idx, 6) =
        reference_rays_sample.row(idx) * current_rays_sample(idx, 2);
  }

  Eigen::JacobiSVD<Eigen::Matrix<float, 8, 9>> svd_solver(A,
                                                          Eigen::ComputeFullV);
  svd_solver.computeV();
  ...
  Eigen::JacobiSVD<Eigen::Matrix<float, 3, 3>> svd_essential_matrix(
      E, Eigen::ComputeFullU | Eigen::ComputeFullV);
  Eigen::Vector3f eigen_values;
  eigen_values << 1, 1, 0;
  Eigen::Matrix<float, 3, 3> Ef = svd_essential_matrix.matrixU() *
                                  eigen_values.asDiagonal() *
                                  svd_essential_matrix.matrixV().transpose();

  return -Ef;
}

Eigen::Matrix3f EssentialMatrixInitialization::RefineSolution(
    Eigen::MatrixXf& reference_rays, Eigen::MatrixXf& current_rays) {
...
int EssentialMatrixInitialization::ComputeScoreAndInliers(
    const int n_matched, Eigen::Matrix<float, 3, 3>& E,
    std::vector<bool>& inliers) {
  Eigen::MatrixXf reference_rays_transformed =
      (E * reference_rays_.block(0, 0, n_matched, 3).transpose())
          .transpose()
          .rowwise()
          .normalized();
  ...
  auto errors =
      (M_PI / 2 -
       (reference_rays_transformed.array() *
        current_rays_.block(0, 0, n_matched, 3).rowwise().normalized().array())
           .rowwise()
           .sum()
           .acos())
          .abs() < options_.epipolar_threshold;
...
absl::Status EssentialMatrixInitialization::ReconstructEnvironment(
  ...
  return ReconstructPoints(camera_transform_world, landmarks_position,
                           essential_matrix_inliers);
}

void EssentialMatrixInitialization::ReconstructCameras(
    Eigen::Matrix3f& E, Sophus::SE3f& camera_transform_world,
    Eigen::MatrixXf& rays_1, Eigen::MatrixXf& rays_2) {
  // Decompose E into 2 rotation hypotheses (R_1 and R_2) and a translation
  // vector.
  ...
  float away = ((R_good * rays_1.transpose() - rays_2.transpose()).array() *
                (rays_2.transpose().colwise() - t).array())
                   .colwise()
                   .sum()
                   .sign()
                   .sum();
...
void EssentialMatrixInitialization::DecomposeEssentialMatrix(
    Eigen::Matrix3f& E, Eigen::Matrix3f& R_1, Eigen::Matrix3f& R_2,
    Eigen::Vector3f& t) {
  ...
  if (R_1.determinant() < 0) R_1 = -R_1;

  R_2 = svd.matrixU() * W * svd.matrixV().transpose();
  if (R_2.determinant() < 0) R_2 = -R_2;

  t = svd.matrixU().col(2).normalized();
}

absl::Status EssentialMatrixInitialization::ReconstructPoints(
  ...
  int n_parallax = 0, n_reprojection_error_1 = 0, n_reprojection_error_2 = 0,
      n_triangulation_error = 0;
  ...
  for (int idx = 0; idx < essential_matrix_inliers.size(); idx++) {
    if (essential_matrix_inliers[idx]) {
      ...
      auto landmark_position_status = TriangulateMidPoint(
          reference_ray, current_ray, Sophus::SE3f(), camera_transform_world);
      if (!landmark_position_status.ok()) {
        landmarks_position[idx] =
            absl::InternalError("Internal triangulation error.");
      ...
      // Check that the point has been triangulated in front of the first camera
      // (positive depth).
      if (landmark_position(2) < 0.0f) {
        landmarks_position[idx] =
            absl::InternalError("Negative depth at first camera.");
        n_depth_1++;
        continue;
      }
      ...
      cv::Point2f projected_landmark_1 =
          calibration_->Project(landmark_position);

      if (SquaredReprojectionError(reference_keypoints_[idx].pt,
                                   projected_landmark_1) > 5.991) {
        landmarks_position[idx] =
            absl::InternalError("High reprojection error at first camera.");
        n_reprojection_error_1++;
        continue;
      }

      Eigen::Vector3f landmark_position_camera2 =
          camera_transform_world * landmark_position;
      if (landmark_position_camera2(2) < 0.0f) {
        landmarks_position[idx] =
            absl::InternalError("Negative depth at second camera.");
        n_depth_2++;
        continue;
      }

      cv::Point2f projected_landmark_2 =
          calibration_->Project(landmark_position_camera2);

      if (SquaredReprojectionError(current_keypoints_[idx].pt,
                                   projected_landmark_2) > 5.991) {
        landmarks_position[idx] =
            absl::InternalError("High reprojection error at second camera.");
