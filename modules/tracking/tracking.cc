...
Tracking::Tracking(const Tracking::Options options, std::shared_ptr<Map> map,
  ...
  monocular_map_initializer_options.rigid_initializer_radians_per_pixel =
      options_.radians_per_pixel;
...
void Tracking::ExtractFeaturesInFrame(const cv::Mat& im, const cv::Mat& mask,
  ...
  for (int idx = 0; idx < tracked_keypoints.size(); idx++) {
    frame.InsertObservation(tracked_keypoints[idx], Eigen::Vector3f::Zero(), 0,
                            TRACKED);
  }
}
...
void Tracking::PointReuse(const cv::Mat& im, const cv::Mat& mask,
  ...
  LucasKanadeTracker klt(
      cv::Size(options_.klt_window_size, options_.klt_window_size), 1,
      options_.klt_max_iters, options_.klt_epsilon, options_.klt_min_eig_th);
  ...
  vector<Eigen::Vector3f> tracked_candidate_landmarks =
      frame_with_only_candidates.GetLandmarkPositionsWithStatus(
          {TRACKED_WITH_3D});
