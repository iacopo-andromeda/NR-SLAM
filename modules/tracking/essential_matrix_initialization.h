...
class EssentialMatrixInitialization {
 public:
  ...
  // Tries to initialize using the reference and current feature tracks. Returns
  // Ok in success and camera_transform_world and landmarks_position hold valid
  // data.
 ...
 private:
  ...
  // Computes the number of inliers of a given Essential matrix with the current
  // data.
  int ComputeScoreAndInliers(const int n_matched, Eigen::Matrix<float, 3, 3>& E,
                             std::vector<bool>& inliers);

  // Environment reconstruction from a given Essential Matrix.
  absl::Status ReconstructEnvironment(
      Eigen::Matrix3f& E, Sophus::SE3f& camera_transform_world,
      std::vector<absl::StatusOr<Eigen::Vector3f>>& landmarks_position,
      int& n_inliers, std::vector<bool>& essential_matrix_inliers);

  // Reconstructs the camera pose from the Essential Matrix. Automatically
  // sellects the correct rotaiton and translation
  void ReconstructCameras(Eigen::Matrix3f& E,
                          Sophus::SE3f& camera_transform_world,
                          Eigen::MatrixXf& rays_1, Eigen::MatrixXf& rays_2);

  // Reconstructs the environment with te predicted camera pose
  absl::Status ReconstructPoints(
      const Sophus::SE3f& camera_transform_world,
      std::vector<absl::StatusOr<Eigen::Vector3f>>& landmarks_position,
      std::vector<bool>& essential_matrix_inliers);

  // Decompose an Essential matrix into the 2 possible rotations and
  // translations
  ...
  int ComputeMaxTries(const float inlier_fraction,
                      const float success_likelihood);
  ...
  Eigen::Matrix<float, Eigen::Dynamic, 3, Eigen::RowMajor> reference_rays_,
      current_rays_;
