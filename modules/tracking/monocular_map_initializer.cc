...
MonocularMapInitializer::MonocularMapInitializer(
    Options& options, std::shared_ptr<Feature> feature_extractor,
...
MonocularMapInitializer::ProcessNewImage(const cv::Mat& im,
  ...
  auto [camera_transform_world, landmarks_positions] =
      *initialization_results_status;

  // Perform a deformable Bundle Adjustment to refine the results.
  return InitializationRefinement(current_keypoints_, landmarks_positions,
                                  feature_labels, camera_transform_world);
}
...
void MonocularMapInitializer::ExtractFeatures(
...
std::vector<int> MonocularMapInitializer::FeatureTracksClustering() {
  ...
  for (const auto& [id, feature_track] :
    ...
    if (feature_track.track_.size() == max_track_length) {
      ...
      for (int idx = 1; idx < feature_track.track_.size(); idx++) {
        cv::Point2f flow =
            feature_track.track_[idx].pt - feature_track.track_[idx - 1].pt;
...
MonocularMapInitializer::RigidInitializationResults
...
MonocularMapInitializer::InitializationRefinement(
    std::vector<cv::KeyPoint>& current_keypoints,
    std::vector<absl::StatusOr<Eigen::Vector3f>>& landmarks_position,
    std::vector<int>& feature_labels, Sophus::SE3f& camera_transform_world) {
  ...
  for (int idx = 0; idx < landmarks_position.size(); idx++) {
    ...
    feature_tracks.push_back(
        feature_tracks_.feature_id_to_feature_track[feature_id].track_);
    track_labels.push_back(feature_labels[idx]);
    vector<Eigen::Vector3f> landmark_track(track_leghth,
  ...
  for (int idx = 0; idx < track_leghth; idx++) {
    ...
    camera_trajectory[idx].translation() =
        camera_transform_world.translation() * weight;
    camera_trajectory[idx].setQuaternion(origin_rotation.slerp(
        weight, camera_transform_world.unit_quaternion()));
  }

  // Build reference and current frames from the estimated geometry.
  auto results = BuildInitializationResults(feature_tracks, landmark_tracks,
                                            track_labels, camera_trajectory);
  results.camera_transform_world = camera_transform_world;
  return results;
}

MonocularMapInitializer::InitializationResults
MonocularMapInitializer::BuildInitializationResults(
    std::vector<std::vector<cv::KeyPoint>>& feature_tracks,
  ...
  for (int idx = 0; idx < feature_tracks.size(); idx++) {
    ...
    initialization_results.reference_landmark_positions.push_back(
        reference_landmark_position);
    initialization_results.current_landmark_positions.push_back(
