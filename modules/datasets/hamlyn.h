...
class Hamlyn {
 public:
  /*
   * Loads the dataset stored at path. If the video has not beeen previously
   * splite, it splits it. Otherwise just loads the images
   */
  Hamlyn(const std::string& video_path,
 ...
 private:
   ...
   * stored at path
   */
  bool SplitVideoIntoFrames(const std::string& path,
                            const std::string& video_path,
                            const std::string& other_video_path = "");

  std::vector<std::string> left_images_names_,
      right_images_names;  // Vector with the image paths.
