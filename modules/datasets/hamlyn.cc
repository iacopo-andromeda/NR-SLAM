...
Hamlyn::Hamlyn(const std::string& video_path,
...
bool Hamlyn::SplitVideoIntoFrames(const std::string& path,
  ...
  leftCal_ = (cv::Mat_<double>(3, 3) << 755.312744, 0.0, 327.875, 0.0,
              420.477722, 165.484406, 0.0, 0.0, 1.0);
  leftDistorsion_ =
      (cv::Mat_<double>(1, 4) << -0.186853, 0.122769, -0.010146, -0.003869);

  rightCal_ = (cv::Mat_<double>(3, 3) << 759.047791, 0.0, 391.990051, 0.0,
               415.329529, 151.748993, 0.0, 0.0, 1);
  rightDistorsion_ =
      (cv::Mat_<double>(1, 4) << -0.197641, 0.213583, -0.00037, -0.010498);

  R = (cv::Mat_<double>(3, 3) << 0.999835, 0.001024, 0.018154, -0.001085,
       0.999994, 0.003314, -0.018151, -0.003333, 0.99983);
  t = (cv::Mat_<double>(3, 1) << -5.196155, -0.030411, 0.212897);
  ...
  /* leftCal_ = (cv::Mat_<double>(3,3) << 381.914307, 0.0, 168.108963,
          ...
          0.0, 0.0, 1.0);
  leftDistorsion_ = (cv::Mat_<double>(1,4) << -0.333236, 0.925076, 0.003847,
  0.000916);

  rightCal_ = (cv::Mat_<double>(3,3) << 381.670013, 0.0, 129.929291,
          0.0, 382.582397, 120.092186,
          0.0, 0.0, 1);
  rightDistorsion_ = (cv::Mat_<double>(1,4) << -0.329342, 0.699034, 0.004927,
  0.008194);
  ...
  cv::stereoRectify(leftCal_, leftDistorsion_, rightCal_, rightDistorsion_,
                    leftImSize, R, t, R_l, R_r, P_l, P_r, Q,
                    cv::CALIB_ZERO_DISPARITY, -1, newSize, 0, 0);
  cv::initUndistortRectifyMap(leftCal_, leftDistorsion_, R_l,
                              P_l.rowRange(0, 3).colRange(0, 3), newSize,
                              CV_32F, M1l, M2l);
  cv::initUndistortRectifyMap(rightCal_, rightDistorsion_, R_r,
                              P_r.rowRange(0, 3).colRange(0, 3), newSize,
                              CV_32F, M1r, M2r);
  ...
  while (true) {
    ...
    if (!has_two_videos) {
      ...
      if (left_image.empty() || right_image.empty()) break;
    }
    ...
    const string left_image_name =
        left_images_path + "/" + to_string(idx) + ".png";
    ...
    const string right_image_name =
        right_images_path + "/" + to_string(idx) + ".png";
