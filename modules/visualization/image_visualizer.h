...
class ImageVisualizer {
 public:
  ...
  void DrawRegularizationGraph(Frame& frame,
                               RegularizationGraph& regularization_graph,
                               const bool use_original_image = true);

  void DrawOpticalFlow(TemporalBuffer& temporal_buffer);

  void DrawClusteredOpticalFlow(
      std::vector<std::vector<cv::Point2f>>& feature_tracks,
      std::vector<int> point_labels, const bool use_original_image = true);
 ...
 private:
  ...
  cv::Scalar HeatMapColor(const float min_value, const float max_value,
                          const float value);

  cv::Scalar OpenCVHeatMapColor(const float min_value, const float max_value,
                                const float value, const int colormap);
