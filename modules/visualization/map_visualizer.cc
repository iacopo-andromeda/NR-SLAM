...
MapVisualizer::MapVisualizer(Options& options, shared_ptr<Map> map)
    : options_(options),
      map_(map),
      last_frame_id_drawn_(-1),
      last_frame_id_saved_(-1) {}
...
#define M1(row, col) m1[(col) * 4 + (row)]
  M1(0, 0) = initial_left_view(0, 0);
  M1(0, 1) = initial_left_view(0, 1);
  M1(0, 2) = initial_left_view(0, 2);
  M1(0, 3) = initial_left_view(0, 3);
  M1(1, 0) = initial_left_view(1, 0);
  M1(1, 1) = initial_left_view(1, 1);
  M1(1, 2) = initial_left_view(1, 2);
  M1(1, 3) = initial_left_view(1, 3);
  M1(2, 0) = initial_left_view(2, 0);
  M1(2, 1) = initial_left_view(2, 1);
  M1(2, 2) = initial_left_view(2, 2);
  M1(2, 3) = initial_left_view(2, 3);
  M1(3, 0) = initial_left_view(3, 0);
  M1(3, 1) = initial_left_view(3, 1);
  M1(3, 2) = initial_left_view(3, 2);
  M1(3, 3) = initial_left_view(3, 3);
#undef M
...
#define M2(row, col) m2[(col) * 4 + (row)]
  M2(0, 0) = initial_right_view(0, 0);
  M2(0, 1) = initial_right_view(0, 1);
  M2(0, 2) = initial_right_view(0, 2);
  M2(0, 3) = initial_right_view(0, 3);
  M2(1, 0) = initial_right_view(1, 0);
  M2(1, 1) = initial_right_view(1, 1);
  M2(1, 2) = initial_right_view(1, 2);
  M2(1, 3) = initial_right_view(1, 3);
  M2(2, 0) = initial_right_view(2, 0);
  M2(2, 1) = initial_right_view(2, 1);
  M2(2, 2) = initial_right_view(2, 2);
  M2(2, 3) = initial_right_view(2, 3);
  M2(3, 0) = initial_right_view(3, 0);
  M2(3, 1) = initial_right_view(3, 1);
  M2(3, 2) = initial_right_view(3, 2);
  M2(3, 3) = initial_right_view(3, 3);
#undef M
...
void MapVisualizer::FinishVisualization() { pangolin::FinishFrame(); }
...
void MapVisualizer::DrawLastFrame() {
  ...
  for (int idx = 0; idx < last_frame.LandmarkPositions().size(); idx++) {
    if (last_frame.LandmarkStatuses()[idx] == TRACKED_WITH_3D) {
      ...
      landmarks_recently_triangulated.push_back(
          last_frame.LandmarkPositions()[idx]);
    }
  }
  ...
  if (*show_ground_truth_) {
    DrawGroundTruth(landmarks_with_3d, landmarks_ground_truth,
                    Eigen::Vector3f(1, 0.5, 0.3));
  }

  DrawCamera(last_frame.CameraTransformationWorld(), Eigen::Vector3f(0, 1, 0));
}

void MapVisualizer::Draw3DPoints(std::vector<Eigen::Vector3f> point_cloud,
...
void MapVisualizer::Draw3DFlow(
    const std::vector<std::vector<Eigen::Vector3f>>& flows,
...
void MapVisualizer::DrawGroundTruth(
  ...
  for (int idx = 0; idx < ground_truth_point_cloud.size(); idx++) {
    if (ground_truth_point_cloud[idx].ok()) {
      ...
      glBegin(GL_LINES);
      glVertex3f(ground_truth_point.x(), ground_truth_point.y(),
                 ground_truth_point.z());
      glVertex3f(estimated_landmark.x(), estimated_landmark.y(),
                 estimated_landmark.z());
      glEnd();

      glBegin(GL_POINTS);
      glVertex3f(ground_truth_point(0), ground_truth_point(1),
                 ground_truth_point(2));
      glEnd();
    }
  }
}

void MapVisualizer::DrawCamera(Sophus::SE3f camera_transformation_world,
                               Eigen::Vector3f color) {
...
void MapVisualizer::SetFinish() { should_finish = true; }
...
void MapVisualizer::DrawKeyFrames() {
  ...
  for (const auto& [id, keyframe] : keyframes) {
    if (n_from_last_drawn >= n_skip) {
      DrawCamera(keyframe->CameraTransformationWorld(),
...
void MapVisualizer::DrawConnectedPoints(
    std::vector<Eigen::Vector3f>& trajectory, const Eigen::Vector3f color) {
