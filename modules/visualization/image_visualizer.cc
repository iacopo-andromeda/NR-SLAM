...
void ImageVisualizer::SetCurrentImage(const cv::Mat& im_original,
                                      const cv::Mat& im_processed) {
...
void ImageVisualizer::DrawCurrentFrame(Frame& frame,
                                       const bool use_original_image) {
  cv::Mat image_to_display = (use_original_image)
                                 ? current_color_image_.clone()
                                 : current_processed_image_.clone();
...
void ImageVisualizer::DrawRegularizationGraph(
    Frame& frame, RegularizationGraph& regularization_graph,
    const bool use_original_image) {
  cv::Mat image_to_display = (use_original_image)
                                 ? current_color_image_.clone()
                                 : current_processed_image_.clone();
  ...
  for (int idx = 0; idx < keypoints.size(); idx++) {
    ...
    for (const auto& [mappoint_id_other, regularization_edge] :
      ...
      if (!frame.MapPointIdToIndex().contains(mappoint_id_other) ||
          frame.LandmarkStatuses()[frame.MapPointIdToIndex().at(
      ...
      auto color = HeatMapColor(regularization_graph.GetMinWeightAllowed(), 1.f,
                                regularization_edge->weight);

      cv::line(image_to_display, keypoints[idx].pt,
               frame.Keypoints()[idx_other].pt, color, 1, cv::LINE_AA);
...
void ImageVisualizer::DrawClusteredOpticalFlow(
    std::vector<std::vector<cv::Point2f>>& feature_tracks,
    std::vector<int> point_labels, const bool use_original_image) {
  cv::Mat image_to_display = (use_original_image)
                                 ? current_color_image_.clone()
                                 : current_processed_image_.clone();
  ...
  for (int idx = 0; idx < feature_tracks.size(); idx++) {
    ...
    for (auto it = next(feature_tracks[idx].rbegin(), 1);
         it != feature_tracks[idx].rend(); it++) {
      cv::line(image_to_display, last_pixel_coordinates, *it, color, 2);
...
void ImageVisualizer::DrawFeatures(
  ...
  for (int idx = 0; idx < keypoints.size(); idx++) {
    if (landmarks_position[idx].ok()) {
      DrawFeature(image_to_display, keypoints[idx].pt, cv::Scalar(0, 255, 0));
    } else if (landmarks_position[idx].status().message() ==
               "Internal triangulation error.") {
      ...
      DrawFeature(image_to_display, keypoints[idx].pt, cv::Scalar(255, 0, 0));
               ...
               "High parallax error.") {
      DrawFeature(image_to_display, keypoints[idx].pt,
                  cv::Scalar(0, 255, 255));  // YELLOW
    } else if (landmarks_position[idx].status().message() ==
                   "Negative depth at first camera." ||
               landmarks_position[idx].status().message() ==
                   "Negative depth at second camera.") {
      DrawFeature(image_to_display, keypoints[idx].pt,
...
void ImageVisualizer::DrawFeature(cv::Mat& im, cv::Point2f uv, cv::Scalar color,
                                  int size) {
  cv::Point2f pt1, pt2;
  pt1.x = uv.x - size;
  pt1.y = uv.y - size;
...
cv::Scalar ImageVisualizer::HeatMapColor(const float min_value,
...
cv::Scalar ImageVisualizer::OpenCVHeatMapColor(const float min_value,
                                               const float max_value,
                                               const float value,
                                               const int colormap) {
...
int ImageVisualizer::GetCurrentImageNumber() { return current_image_number_; }
