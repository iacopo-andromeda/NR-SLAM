...
bool ReprojectionError::read(std::istream& is) { return true; };

bool ReprojectionError::write(std::ostream& os) const { return true; };

void ReprojectionError::computeError() {
  const g2o::VertexSE3Expmap* pose_vertex =
...
void ReprojectionError::linearizeOplus() {
  ...
  exponential_map_jacobian << 0.f, landmark_camera_position.z(),
      -landmark_camera_position.y(), 1.f, 0.f, 0.f,
      -landmark_camera_position.z(), 0.f, landmark_camera_position.x(), 0.f,
      1.f, 0.f, landmark_camera_position.y(), -landmark_camera_position.x(),
      0.f, 0.f, 0.f, 1.f;

  _jacobianOplusXi = projection_jacobian * exponential_map_jacobian;
  _jacobianOplusXj = projection_jacobian *
                     camera_transformation_world.rotation().toRotationMatrix();
}