...
bool SpatialRegularizerWithObservation::read(std::istream& is) { return true; };
...
void SpatialRegularizerWithObservation::computeError() {
  const LandmarkVertex* current_landmark_vertex =
      static_cast<const LandmarkVertex*>(_vertices[0]);
  const LandmarkVertex* next_landmark_vertex =
      static_cast<const LandmarkVertex*>(_vertices[1]);

  Eigen::Vector3d current_landmark_position =
      current_world_transform_camera_ * current_landmark_vertex->estimate();
  Eigen::Vector3d next_landmark_position =
      next_world_transform_camera_ * next_landmark_vertex->estimate();

  Eigen::Vector3d obs(_measurement);

  _error =
      weight_ * (obs - (next_landmark_position - current_landmark_position));
}
