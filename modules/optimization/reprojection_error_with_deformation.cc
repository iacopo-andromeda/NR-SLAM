...
bool ReprojectionErrorWithDeformation::read(std::istream& is) { return true; };
...
void ReprojectionErrorWithDeformation::computeError() {
  ...
  const LandmarkVertex* deformation_vertex =
...
void ReprojectionErrorWithDeformation::linearizeOplus() {
  ...
  Eigen::Matrix<double, 2, 3> projection_jacobian =
      -calibration_->ProjectionJacobian(landmark_camera);

  Eigen::Matrix<double, 3, 6> exponential_map_jacobian;
  exponential_map_jacobian << 0.f, landmark_camera.z(), -landmark_camera.y(),
      1.f, 0.f, 0.f, -landmark_camera.z(), 0.f, landmark_camera.x(), 0.f, 1.f,
      0.f, landmark_camera.y(), -landmark_camera.x(), 0.f, 0.f, 0.f, 1.f;
