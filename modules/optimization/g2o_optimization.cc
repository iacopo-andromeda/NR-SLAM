...
void CameraPoseOptimization(
  ...
  camera_pose_vertex->setEstimate(
      g2o::SE3Quat(camera_transform_world.unit_quaternion().cast<double>(),
  ...
  vector<cv::KeyPoint> keypoints =
      frame.GetKeypointsWithStatus({TRACKED_WITH_3D});
  vector<Eigen::Vector3f> landmark_positions =
      frame.GetLandmarkPositionsWithStatus({TRACKED_WITH_3D});
  vector<ID> landmark_ids = frame.GetMapPointsIdsWithStatus({TRACKED_WITH_3D});

  vector<ReprojectionErrorOnlyPose*> reprojection_error_edges(keypoints.size(),
                                                              nullptr);

  for (int idx = 0; idx < keypoints.size(); idx++) {
    cv::Point2f pixel_coordinates = keypoints[idx].pt;
    Eigen::Matrix<double, 2, 1> observation(pixel_coordinates.x,
                                            pixel_coordinates.y);
...
absl::flat_hash_set<ID> CameraPoseAndDeformationOptimization(
  ...
  Sophus::SE3f camera_transform_world =
      current_frame.CameraTransformationWorld();
  camera_pose_vertex->setEstimate(
      g2o::SE3Quat(camera_transform_world.unit_quaternion().cast<double>(),
                   camera_transform_world.translation().cast<double>()));
  camera_pose_vertex->setId(0);
  ...
  const float info_reprojection =
      1.0f / (sigma_reprojection * sigma_reprojection);
  ...
  vector<absl::flat_hash_map<int, SpatialRegularizerWithDeformation*>>
      spatial_regularizers(points_in_optimization);
  vector<absl::flat_hash_map<int, PositionRegularizerWithDeformation*>>
      position_regularizers(points_in_optimization);
  vector<ReprojectionErrorWithDeformation*> reprojection_errors(
      points_in_optimization);
  ...
  for (int idx = 0; idx < points_in_optimization; idx++) {
    ...
    ReprojectionErrorWithDeformation* reprojection_error =
        new ReprojectionErrorWithDeformation();
    ...
    reprojection_error->setVertex(
        0, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(0)));
    reprojection_error->setVertex(1,
                                  dynamic_cast<g2o::OptimizableGraph::Vertex*>(
                                      optimizer.vertex(idx + 1)));

    reprojection_error->setInformation(Eigen::Matrix2d::Identity() *
                                       info_reprojection);
    ...
    for (const auto& [mappoint_id_other, regularization_edge] :
         regularization_edges) {
      // Check if there is already enough regularizers or if the connection is
      // good.
      ...
      // Check if this regularizer has already been inserted in the
      // optimization.
      ...
      Eigen::Matrix3d spatial_information_matrix =
          Eigen::Matrix3d::Identity() * info_spatial;
      ...
      PositionRegularizerWithDeformation* position_regularizer =
          new PositionRegularizerWithDeformation();
      position_regularizer->setVertex(
          0, dynamic_cast<g2o::OptimizableGraph::Vertex*>(
                 optimizer.vertex(idx + 1)));
      position_regularizer->setVertex(
          1, dynamic_cast<g2o::OptimizableGraph::Vertex*>(
      ...
      position_regularizer->rest_position_1_ =
          landmark_positions[idx].cast<double>();
      position_regularizer->rest_position_2_ =
          landmark_positions[idx_other].cast<double>();

      g2o::RobustKernelHuber* robust_kernel_position =
  ...
  for (int iteration = 0; iteration < iterations.size(); iteration++) {
    ...
    for (int idx = 0; idx < points_in_optimization; idx++) {
      ...
      if (chi_squared > th_huber_2dof_squared) {
        ...
        for (auto& [idx_other, spatial_regularizer] :
             spatial_regularizers[idx]) {
  ...
  for (int idx = 0; idx < points_in_optimization; idx++) {
    LandmarkVertex* deformation_vertex =
        static_cast<LandmarkVertex*>(optimizer.vertex(idx + 1));
  ...
  for (int idx = 0; idx < points_in_optimization; idx++) {
    ...
    Eigen::Vector3f current_landmark_position =
        deformation + previous_landmark_position;

    current_frame.LandmarkPositions()[index_in_frame] =
        current_landmark_position;

    map->GetMapPoint(mappoints_ids[idx])
        ->SetLastWorldPosition(current_landmark_position);
  ...
  for (ID lost_mappoint_id : lost_mappoint_ids_ordered) {
    ...
    for (const auto& [mappoint_id_other, regularization_edge] :
      ...
      SpatialRegularizerFixed* spatial_regularizer =
          new SpatialRegularizerFixed();
      spatial_regularizer->setVertex(
          0, dynamic_cast<g2o::OptimizableGraph::Vertex*>(
  ...
  for (const auto& [mappoint_id, idx] : lost_mappoint_id_to_index) {
    ...
    LandmarkVertex* deformation_vertex =
        static_cast<LandmarkVertex*>(optimizer.vertex(idx));
    Eigen::Vector3f deformation = deformation_vertex->estimate().cast<float>();

    Eigen::Vector3f previous_landmark_position =
        mappoint->GetLastWorldPosition();
    Eigen::Vector3f current_landmark_position =
        deformation + previous_landmark_position;
...
absl::StatusOr<Eigen::Vector3f> DeformableTriangulation(
  ...
  auto neighbour_ids =
      temporal_buffer.GetClosestMapPointsToFeature(candidate_id, 10, 20, 500);
  ...
  absl::flat_hash_map<int, ReprojectionErrorOnlyDeformation*>
      frame_id_to_reprojection_edge;
  ...
  Eigen::Vector3f landmark_current_position =
      (*current_camera_transform_world) * (*landmark_position_status);
  cv::Point2f projected_landmark_1 =
      calibration->Project(landmark_current_position);

  if (SquaredReprojectionError(current_keypoint.pt, projected_landmark_1) >
      5.991) {
    return absl::InternalError("High reprojection error at first camera.");
  }

  Eigen::Vector3f landmark_previous_position =
      (*previous_camera_transform_world) * (*landmark_position_status);
  cv::Point2f projected_landmark_2 =
      calibration->Project(landmark_previous_position);

  if (SquaredReprojectionError(previous_keypoint.pt, projected_landmark_2) >
      5.991) {
  ...
  for (const auto& [frame_id, keypoint] : candidate_track) {
    auto camera_transform_world =
        temporal_buffer.GetCameraTransformWorld(frame_id);
    CHECK_OK(camera_transform_world);

    float depth_seed = 0.0f;
    int n_neighbors = 0;
    for (const auto neighbor_id : neighbour_ids) {
      auto landmark_position =
          temporal_buffer.GetLandmarkPosition(frame_id, neighbor_id);
    ...
    Eigen::Vector3d landmark_position_seed =
        (calibration->Unproject(keypoint.pt) * depth_seed).cast<double>();
    ...
    Eigen::Matrix2d informationMatrix =
        Eigen::Matrix2d::Identity() * info_reprojection;
  ...
  for (auto current_iterator = candidate_track.begin();
    ...
    g2o::SE3Quat current_pose_g2o =
        g2o::SE3Quat((*current_camera_transform_world)
                         .inverse()
                         .unit_quaternion()
                         .cast<double>(),
                     (*current_camera_transform_world)
    ...
    for (auto next_iterator = next(current_iterator);
      ...
      auto next_camera_transform_world =
          temporal_buffer.GetCameraTransformWorld(next_frame_id);
      CHECK_OK(next_camera_transform_world);

      g2o::SE3Quat next_pose_g2o = g2o::SE3Quat((*next_camera_transform_world)
                                                    .inverse()
                                                    .unit_quaternion()
                                                    .cast<double>(),
                                                (*next_camera_transform_world)
                                                    .inverse()
                                                    .translation()
                                                    .cast<double>());

      for (auto neighbor_id : neighbour_ids) {
        auto current_landmark_position =
            temporal_buffer.GetLandmarkPosition(current_frame_id, neighbor_id);
        auto next_landmark_position =
            temporal_buffer.GetLandmarkPosition(next_frame_id, neighbor_id);

        auto first_landmark_position =
            temporal_buffer.GetLandmarkPosition(first_frame_id, neighbor_id);
        ...
        Eigen::Vector3f flow =
            (*next_landmark_position) - (*current_landmark_position);
  ...
  auto last_camera_transform_world =
      temporal_buffer.GetCameraTransformWorld(last_frame_id);
  CHECK_OK(last_camera_transform_world);

  Eigen::Vector3f unprojected_keypoint =
      calibration->Unproject(latest_pixel_coordinates);
  unprojected_keypoint /= unprojected_keypoint.z();

  Eigen::Vector3f triangulated_landmark_position =
      ((*last_camera_transform_world).inverse() *
       (unprojected_keypoint * current_depth));
...
class TemporalPoint {
 public:
  ...
  TemporalPoint(ID mappoint_id_1, ID mappoint_id_2, ID keyframe_id_1,
                ID keyframe_id_2) {
  ...
  bool operator==(const TemporalPoint& rhs) const {
    return this->mappoint_id_1_ == rhs.mappoint_id_1_ &&
           this->mappoint_id_2_ == rhs.mappoint_id_2_ &&
           this->keyframe_id_1_ == rhs.keyframe_id_1_ &&
           this->keyframe_id_2_ == rhs.keyframe_id_2_;
  }
  ...
  struct HashFunction {
    size_t operator()(const TemporalPoint& point) const {
      ...
      return mappoint_1_hash ^ mappoint_2_hash ^ keyframe_1_hash ^
             keyframe_2_hash;
    }
  };
...
void LocalDeformableBundleAdjustment(std::shared_ptr<Map> map,
  ...
  for (auto it = keyframes_in_optimization.rbegin();
       it != keyframes_in_optimization.rend(); it++) {
    auto keyframe = *it;
    ID keyframe_id = keyframe->GetId();

    auto landmark_positions =
  ...
  // Point vertices will be added as we add observations. This map stores the
  // landmarks vertex index in the optimization in the following manner:
  ...
  for (auto it = keyframes_in_optimization.rbegin();
    ...
    for (int idx = 0; idx < landmark_positions.size(); idx++) {
      ...
      int landmark_optimization_index =
          inserted_landmarks[keyframe_id][mappoint_id];
      ...
      for (const auto& [mappoint_id_other, regularization_edge] :
        ...
        // Check if this regularizer has already been inserted in the
        // optimization.
        SpatialPoint spring_connection(mappoint_id, mappoint_id_other,
                                       keyframe_id);
        ...
        position_regularizer->setMeasurement(
            regularization_edge->first_distance);
      ...
      if (next_keyframe) {
        ...
        for (const auto& [mappoint_id_other, regularization_edge] :
             regularization_edges) {
          // Check if there is already enough regularizers or if the connection
          // is good.
          if (n_regularizers > regularizers_per_point ||
              regularization_edge->status == RegularizationGraph::BAD) {
            break;
          ...
          if (!inserted_landmarks[keyframe_id].contains(mappoint_id_other) ||
              !inserted_landmarks[next_keyframe_id].contains(
                  mappoint_id_other)) {
            continue;
          }
          ...
          // Check if this regularizer has already been inserted in the
          // optimization.
          TemporalPoint dumper_connection(mappoint_id, mappoint_id_other,
          ...
          spatial_regularizer->setVertex(
              1, dynamic_cast<g2o::OptimizableGraph::Vertex*>(
                     optimizer.vertex(other_landmark_optimization_index)));
          spatial_regularizer->setVertex(
              2, dynamic_cast<g2o::OptimizableGraph::Vertex*>(
                     optimizer.vertex(next_landmark_optimization_index)));
          spatial_regularizer->setVertex(
              3, dynamic_cast<g2o::OptimizableGraph::Vertex*>(
  ...
  for (const auto [keyframe_id, mappoint_id_to_idx] : inserted_landmarks) {
    ...
    auto keyframe_vertex =
        static_cast<g2o::VertexSE3Expmap*>(optimizer.vertex(keyframe_id));
    ...
    for (const auto [mappoint_id, idx_in_optimization] : mappoint_id_to_idx) {
      auto landmark_vertex =
          static_cast<LandmarkVertex*>(optimizer.vertex(idx_in_optimization));

      int idx_in_keyframe = keyframe->MapPointIdToIndex().at(mappoint_id);
      keyframe->LandmarkPositions()[idx_in_keyframe] =
          landmark_vertex->estimate().cast<float>();
    }
  }
}
