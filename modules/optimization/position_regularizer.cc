...
bool PositionRegularizer::read(std::istream& is) { return true; }

bool PositionRegularizer::write(std::ostream& os) const { return true; }

void PositionRegularizer::computeError() {
  const LandmarkVertex* vertex_landmark_1 =
      static_cast<const LandmarkVertex*>(_vertices[0]);
  const LandmarkVertex* vertex_landmark_2 =
...
void PositionRegularizer::linearizeOplus() {
  ...
  const Eigen::Vector3d difference_vector =
