...
bool ShiTomasi::IsLocalMaximum(int r, int c) {
  ...
  for (int i = minRow; i <= maxRow; i++) {
    for (int j = minCol; j <= maxCol; j++) {
      if (scores.at<float>(i, j) == -1.f) return false;

      if (i >= minRow_inner && i <= maxRow_inner && j >= minCol_inner &&
          j <= maxCol_inner) {
        if (scores.at<float>(i, j) > currValue) return false;
      }
    }
  }
...
void ShiTomasi::FastSobelXYandScore(const cv::Mat& im) {
  ...
  // Y GRAD LAST COLUMN
  r1[cols_l - 1] = pIm[0][cols_l - 1] + pIm[0][cols_l - 1] +
                   pIm[0][cols_l - 2] + pIm[0][cols_l - 2];
  r2[cols_l - 1] = pIm[1][cols_l - 1] + pIm[1][cols_l - 1] +
                   pIm[1][cols_l - 2] + pIm[1][cols_l - 2];
  r3[cols_l - 1] = pIm[2][cols_l - 1] + pIm[2][cols_l - 1] +
                   pIm[2][cols_l - 2] + pIm[2][cols_l - 2];
...
void ShiTomasi::DetectCorner(int col) {
  if (col == 1) {  // There are not any precomputed information
    ...
    // Compute short term data
    //-------------------------------
    _G11_c1 = pXgrad[0][col] * pXgrad[0][col] +
              pXgrad[1][col] * pXgrad[1][col] + pXgrad[2][col] * pXgrad[2][col];
    _G12_c1 = pXgrad[0][col] * pYgrad[0][col] +
              pXgrad[1][col] * pYgrad[1][col] + pXgrad[2][col] * pYgrad[2][col];
    _G22_c1 = pYgrad[0][col] * pYgrad[0][col] +
              pYgrad[1][col] * pYgrad[1][col] + pYgrad[2][col] * pYgrad[2][col];

    _G11_c2 = pXgrad[0][col + 1] * pXgrad[0][col + 1] +
              pXgrad[1][col + 1] * pXgrad[1][col + 1] +
              pXgrad[2][col + 1] * pXgrad[2][col + 1];
    _G12_c2 = pXgrad[0][col + 1] * pYgrad[0][col + 1] +
              pXgrad[1][col + 1] * pYgrad[1][col + 1] +
              pXgrad[2][col + 1] * pYgrad[2][col + 1];
    _G22_c2 = pYgrad[0][col + 1] * pYgrad[0][col + 1] +
              pYgrad[1][col + 1] * pYgrad[1][col + 1] +
              pYgrad[2][col + 1] * pYgrad[2][col + 1];

    //-------------------------------
    // Compute spatial tensor
    //-------------------------------
    tensor[0] = (_G11_c1 + _G11_c2 + pXgrad[0][col - 1] * pXgrad[0][col - 1] +
                 pXgrad[1][col - 1] * pXgrad[1][col - 1] +
                 pXgrad[2][col - 1] * pXgrad[2][col - 1]) *
                inv_size;
    tensor[1] = (_G12_c1 + _G12_c2 + pXgrad[0][col - 1] * pYgrad[0][col - 1] +
                 pXgrad[1][col - 1] * pYgrad[1][col - 1] +
                 pXgrad[2][col - 1] * pYgrad[2][col - 1]) *
                inv_size;
    tensor[2] = (_G22_c1 + _G22_c2 + pYgrad[0][col - 1] * pYgrad[0][col - 1] +
                 pYgrad[1][col - 1] * pYgrad[1][col - 1] +
                 pYgrad[2][col - 1] * pYgrad[2][col - 1]) *
                inv_size;
