    ...
    while (getline(filterFile, line)) {
      ...
      if (name == "BorderFilter") {
        ...
        std::unique_ptr<Filter> f(
            new BorderFilter(stoi(rb), stoi(re), stoi(cb), stoi(ce), stoi(th)));
        addFilter(f);
...
absl::flat_hash_map<std::string, cv::Mat> Masker::GetAllMasks(
    const cv::Mat& im) {
  ...
  LOG(INFO) << "Generating masks for image of size: " << im.cols << "x"
            << im.rows;
